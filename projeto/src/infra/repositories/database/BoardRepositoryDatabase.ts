    ...
    async save(board: Board): Promise<number> {
        ...
        for(const column of board.columns){
            ...
            for(const card of column.cards){
                await this.connection.query("insert into kanban.card (id_board, id_column, title, estimative) values ($1, $2, $3, $4)",[boardData.id_board, columnData.id_column, card.name, card.estimative])
            }
        }
        return boardData.id_board
    }
    async get(idBoard: number): Promise<Board | undefined> {
        const [boardData] = await this.connection.query("select * from kanban.board where id_board = $1",[idBoard])
        if(!boardData) throw new Error("Board does not exists")
